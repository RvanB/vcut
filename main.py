...
def main():
    ...
    parser.add_argument("-i", "--image-file", help="Path to the input image", required=True)
    parser.add_argument("-o", "--output-file", help="Path to the output normal map", required=True)
    parser.add_argument("-u", "--upscale-factor", type=float, help="How much to upscale the image prior to normal generation")
    parser.add_argument("-t", "--threshold", help="Threshold for mask after upscaling", default=40)
    parser.add_argument("-s", "--tangent-sample-size", help="How far forward and backward to look on contour for tangent estimation")
...
@njit(parallel=True)
def compute_normals(mask, contour_x, contour_y, contour_id_map, contour_index_map,
                    contours_flat, contour_start_idx, nx2d, ny2d, tangent_sample_size=4, crotch_smoothing_threshold=0.2):
    h, w = mask.shape
    for y in prange(h):
        for x in range(w):
            if not mask[y, x]:
            ...
            # if abs(a1 - a2) < crotch_smoothing_threshold:
            #     ratio = a1 / a2
            #     norm_x = n1x * (1 - ratio) + n2x * ratio
            #     norm_y = n1y * (1 - ratio) + n2y * ratio
...
def compute_contour_based_normals(mask, strength=1.0):
    ...
    compute_normals(mask, contour_x, contour_y, contour_id_map, contour_index_map,
                    contours_flat, contour_start_idx, nx2d, ny2d, 8, 0.2)
